...
#### 范围：平台自己发过的 + 线上所有未来排期，不含店铺历史

用户明确要求：**「只存平台自己发布的 和未来的，过去的通通不记录」**。

店铺里平台上线之前就存在的已发布历史（实测 2748 条）**不入库**。

「未来的」这半句很关键：**排期不是平台独有的事** —— 内容可能是在 Shopify
后台、或别的工具排上去的。实测店铺里有 **172 条**这样的排期页面
（community_post 156 / discord-page 15 / nas-a-vs-b 1），平台一条都不知道。
不拉进来，仪表盘的「排期全景」就是残缺的。

### 同步：拉未来排期 + 对账

`POST /api/sync/run` 做两件事：

**1. 拉未来排期**

用 `published_status:unpublished` 过滤（已验证该过滤器有效），只收时间在
**未来**的条目，草稿（未发布且没有未来时间）不收。实测 12 文章 + 184 页面，
**一次请求**搞定。

归属判断：文章看 blog handle、页面看 `templateSuffix`
...
平台发定时内容用的是 Shopify 原生机制（未来 `publishDate` + `isPublished: false`），
**到点由 Shopify 自己上线**。好处是本地不需要定时任务，服务没开也不会漏发；
代价是线上到点发生的事本地不会自动知道。不对账会看到三种假象：
...
- **`published_at` 只在真正已发布时写**：Shopify 对未发布的排期项也会返回一个
  未来的 `publishedAt`，写进 `published_at` 会让时间轴显示成「已发布」，
  还会与拉取写入的 `None` 冲突、每次对账都报「更新了 N 条」
- 时间比较前先归一化：Shopify 回 `...Z`，本地可能存 `+00:00`
- 拉取**按 GID 优先匹配**已有行，否则平台自己发过的对象会出现两行
- 对账发现对象消失时**本地留痕不删行**（写 `error`）
- 接口报错时如实上报且**不动本地数据**
...
| `pnpm dev` | 后端真实数据（**默认**） | 日常使用、端到端联调 |
| `pnpm dev:mock` | 内置演示数据（`.env.mock`） | 后端没起时看界面 |

对应的验收脚本：

| 命令 | 跑在哪个模式 | 验什么 |
|---|---|---|
| `pnpm verify:ui` | 演示模式（需 `pnpm dev:mock` 起服务） | UI 渲染、侧边栏、解析校验、选择器、无运行时报错 |
| `pnpm verify:real` | 真实模式（`pnpm dev`） | 「演示模式角标消失」、统计卡与库一致、历史记录读到真实数据 |
